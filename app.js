...
$(function() {
    ...
    $('#hide_unselected').click(function() {
        if ($(this).children('input').is(':checked')) {
            $('.class:not(.active)').addClass('inactive');
            $('.class.active.repeat').addClass('repeat_hide');
        } else {
            $('.class:not(.active)').removeClass('inactive');
            $('.class.active.repeat').removeClass('repeat_hide');
        }
    });
    ...
    $('#hide_full').click(function() {
        ...
        if ($(this).children('input').is(':checked')) {
            ...
            $('.class .capacity.warning').parent().addClass('full');
            
            /* prevent orphaned indented lists; go through each 1st level
             * course and check if it has children - if it does, then select
             * all of the children, and remove the repeat class on the first
             * non-full child
             */
        ...
        // deactivate the hidden full classes option
        else {
        
            $('.class .capacity.warning').parent().removeClass('full');
            
            /* undo "prevent orphaned indented lists" above
             */
            $('.class:not(.repeat) .capacity.warning').parent().each(
                function() {
                    if ($(this).next().hasClass('repeat')) {
